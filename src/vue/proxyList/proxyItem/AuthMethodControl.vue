<template>
  <div class="auth-method-control">
    <template v-if="isEditing">
      <div class="auth-method-control__edit-block">
        <template v-if="editType === 'ip'">
          <input type="text" class="input" v-model="editIp" >
        </template>
        <template v-else>
          <input type="text" class="input" v-model="editLogin">
          <input type="password" class="input" v-model="editPass">
        </template>
      </div>
      <div class="auth-method-control__actions">
        <button class="btn-icon btn-icon--primary" @click="save">
    ...
    <template v-else>
      <div class="auth-method-control__credentials">
        <div v-if="editType === 'ip'">
          {{ editIp }}
        </div>
        <div v-else>
          <div>{{ editLogin }}</div>
        ...
        <button class="btn-icon btn-icon--link" @click="isEditing=!isEditing">
          <svg width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <mask id="mask0_421_2859" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="1" y="2" width="14"
                  height="15">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M1.50024 2.75H14.3857V16.9547H1.50024V2.75Z"
                    fill="white"/>
            </mask>
      ...
      <div class="auth-method-control__type-select">
        <label class="radio-item">
          <input type="radio"
                 :name="`${proxyId}-auth-type`"
                 value="ip"
                 v-model="editType">
          <span>IP</span>
        </label>
...
export default {
  ...
  mounted() {
    ...
    this.editIp = this.ip
    this.editLogin = this.login
    this.editPass = this.password
  }
}
</script>
